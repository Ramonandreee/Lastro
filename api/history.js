...
const isoDay = (ms) => { const d = new Date(ms); const p = (n) => String(n).padStart(2, '0'); return `${d.getUTCFullYear()}-${p(d.getUTCMonth() + 1)}-${p(d.getUTCDate())}`; };
...
// B3 via brapi: histórico diário por lote (range 1mo, interval 1d).
async function brapiHistory(symbols, token, out) {
  if (!symbols.length || !token) return;
  await mapLimit(chunk(symbols, BATCH), 3, async (grp) => {
    const path = grp.map(encodeURIComponent).join(',');
    const d = await fetchJson(`${BRAPI}/quote/${path}?range=1mo&interval=1d&token=${encodeURIComponent(token)}`);
    const results = (d && d.results) || [];
    results.forEach((q) => {
      const hist = q && Array.isArray(q.historicalDataPrice) ? q.historicalDataPrice : [];
      const ser = hist.map((h) => ({ d: isoDay((h.date || 0) * 1000), c: num(h.close) })).filter((x) => x.c != null && x.c > 0);
      if (ser.length) out[String(q.symbol).toUpperCase()] = dedupeAsc(ser);
    });
  });
}
...
export default async function handler(req, res) {
  const q = req.query || {};
  const days = Math.min(Math.max(parseInt(q.days, 10) || 40, 7), 90);
  ...
  try {
    await Promise.all([
      brapiHistory(symbols, process.env.BRAPI_TOKEN || '', out),
    ...
    return res.status(200).json({ results: out, error: String((e && e.message) || e) });
  }
}
