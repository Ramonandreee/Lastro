...
export function withObs(ep, handler) {
  return async function (req, res) {
    ...
    try {
      ...
      res.setHeader('X-Content-Type-Options', 'nosniff');   // defesa contra MIME-sniffing (o Content-Type é sempre explícito)
      res.setHeader('Referrer-Policy', 'no-referrer');      // não vaza a URL do proxy p/ terceiros
    } catch { /* headers podem já ter saído */ }
    const t0 = Date.now();
    try {
