...
// Câmbio (forex) via FMP: historical-price-full/USDBRL → { symbol, historical:[...] }.
// Usado p/ dolarizar meses PASSADOS no câmbio real (não o câmbio de hoje).
async function fmpForex(pairs, key, days, out) {
  if (!pairs.length || !key) return;
  await mapLimit(pairs, 3, async (pair) => {
    const sym = String(pair).toUpperCase().replace('/', '');
    const d = await fetchJson(`${FMP}/historical-price-full/${encodeURIComponent(sym)}?serietype=line&timeseries=${days}&apikey=${encodeURIComponent(key)}`);
    const hist = d && (Array.isArray(d.historical) ? d.historical
      : (Array.isArray(d.historicalStockList) && d.historicalStockList[0] && Array.isArray(d.historicalStockList[0].historical) ? d.historicalStockList[0].historical : []));
    const ser = (hist || []).map((h) => ({ d: String(h.date).slice(0, 10), c: num(h.close ?? h.price) })).filter((x) => x.c != null && x.c > 0);
...
export default async function handler(req, res) {
  ...
  const fx = parse(q.fx).slice(0, 6);   // pares forex (ex.: USDBRL) p/ dolarizar meses passados no câmbio real
  const out = {};
  try {
    await Promise.all([
      ...
      fmpForex(fx, process.env.FMP_KEY || '', days, out),
