...
- **Gráfico "Patrimônio Total" (Início) + seletor de período:** mostra a **evolução real do patrimônio** com seletor **7D / 30D / 3M / 6M / 1A / MAX** (segmented control estilo iOS, `.whero-periods`/`.wper`; padrão **7D**, memoriza a escolha na sessão via `sessionStorage['lastro_wperiod']` → `homeWealthPeriod`). Troca imediata com animação suave e destaque do período ativo (`setWealthPeriod`). `wealthSeries(period)` usa **fonte única por janela**: primária = a **tabela de histórico de patrimônio** (snapshots diários `patHistArr`); se houver <2 registros (usuário novo), faz **bootstrap por fechamentos reais da B3** (`HIST_CLOSE`/`closeOn`, `/api/market?fn=history`), respeitando data/preço de cada compra. Nunca mistura as duas fontes numa mesma série (evita degraus quando a composição muda). **MAX** = desde o 1º registro; período sem histórico suficiente mostra só o disponível (sem erro). Linha suave em `initWealthSpark`. **Obs.: `wealthSeries()`/`initWealthSpark()` são editadas pelos dois donos — coordenar antes de mexer.**
