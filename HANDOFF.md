...
- **Gráfico "Patrimônio Total" (Início):** mostra a **evolução real dos últimos 7 dias** (um ponto por dia, começa 7 dias atrás e termina hoje), reconstruída em `wealthSeries()` a partir dos **fechamentos reais da B3** (`HIST_CLOSE`/`closeOn`, alimentados por `/api/market?fn=history`), respeitando a data e o preço de cada compra — nunca uma linha reta baseada só no valor atual. Carteira com menos de 7 dias mostra só os dias disponíveis desde a 1ª compra. Linha suave (`initWealthSpark`). **Obs.: `wealthSeries()` é editada pelos dois donos — coordenar antes de mexer.**
