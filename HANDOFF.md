...
- **UX premium:** **apresentação animada de entrada** (`#intro`, estilo Nomad — slides em carrossel com marca, tickers flutuantes, gauge do Score e carteira/IA, autoavanço com barras de progresso, CTAs "Fazer cadastro"/"Entrar"), **Face ID / biometria** (WebAuthn: prompt "Ative o Face ID" no 1º login + tela de desbloqueio ao abrir, com fallback seguro), tour guiado, onboarding, busca global (⌘K), dark mode, PWA instalável, tooltips de termos como cards clicáveis, barras de abas sticky, zoom/seleção desativados no mobile (feel de app nativo).
